...
def plot_w72_variables(df_output):
    ...
    df_output["DOY"] = pd.to_datetime(df_output.day).dt.dayofyear
    ...
    axs[0].set_ylim(0, 20)
    axs[0].plot(df_output.DOY, df_output.tdm * kg_to_Mg)

    axs[1].set_xlabel("Day of year")
    axs[1].set_ylabel("Leaf arae index\n" + r"($\mathrm{m}^{2}$ $\mathrm{m}^{-2}$)")
    axs[1].set_ylim(0, 6)
    axs[1].plot(df_output.DOY, df_output.LAI)

    axs[2].set_xlabel("Day of year")
    axs[2].set_ylabel("Tuber dry matter\n" + r"($\mathrm{Mg}$ DM $\mathrm{ha}^{-1}$)")
    axs[2].set_ylim(0, 20)
    axs[2].plot(df_output.DOY, df_output.WSO * kg_to_Mg )
...
def plot_w81_soil_variables(df_output, Thickness):
    ...
    axs[0,0].set_ylabel("$\mathrm{NH}_4^{+}$-$\mathrm{N}$ amount\n" + r"($\mathrm{kg}$ $\mathrm{NH}_4^{+}$-$\mathrm{N}$ $\mathrm{ha}^{-1}$)", fontsize = 20)
    axs[1,0].set_ylabel("$\mathrm{NO}_3^{-}$-$\mathrm{N}$ amount\n" + r"($\mathrm{kg}$ $\mathrm{NO}_3^{-}$-$\mathrm{N}$ $\mathrm{ha}^{-1}$)", fontsize = 20)
    axs[2,0].set_ylabel("Soil moisture content\n" + r"($\mathrm{m}^{3}$ $\mathrm{H}_{2}\mathrm{O}$ $\mathrm{m}^{-3}$ soil)", fontsize = 20)
...
def plot_w81_crop_variables(df_output):
    ...
    axs[0].set_xlabel("Day of year")
    axs[0].set_ylabel("Crop N amount" + r"($\mathrm{kg} N$ $\mathrm{ha}^{-1}$)")
    axs[0].set_ylim(0, 300)
    axs[0].plot(df_output.DOY, df_output.NamountTot )

    axs[1].set_xlabel("Day of year")
    axs[1].set_ylabel("Leaf N amount\n" + r"($\mathrm{kg}$ N $\mathrm{ha}^{-1}$)")
    axs[1].set_ylim(0, 300)
    axs[1].plot(df_output.DOY, df_output.NamountLV )

    axs[2].set_xlabel("Day of year")
    axs[2].set_ylabel("Tuber N amount\n" + r"($\mathrm{kg}$ N $\mathrm{ha}^{-1}$)")
    axs[2].set_ylim(0, 300)
    axs[2].plot(df_output.DOY, df_output.NamountSO)
...
def plot_w81_soil_totals(df_output, Thickness, number_of_layers):
    ...
    for j in range(0, number_of_layers):
        df_output["NH4N_tot"] += df_output[f"NH4_{j+1}"]
        df_output["NO3N_tot"] += df_output[f"NO3_{j+1}"]
        df_output["H2O_tot"] += df_output[f"SM_{j+1}"] * Thickness[j]

    fig, axs = plt.subplots(1,3, layout = "constrained")
    fig.set_figheight(5)
    fig.set_figwidth(15)
    axs[0].plot(df_output["DOY"], df_output.NH4N_tot * ha_to_m2)
    axs[0].set_xlabel("Day of year", fontsize=15)
    axs[0].set_ylabel("Total amount of ${\mathrm{NH}_4}^{+}$-$\mathrm{N}$\n" + r"($\mathrm{kg}$ ${\mathrm{NH}_4}^{-}$-$\mathrm{N}$  $\mathrm{ha}^{-1}$)", fontsize=15)
    axs[0].set_ylim(0,200)
    axs[1].plot(df_output["DOY"], df_output.NO3N_tot * ha_to_m2)
    axs[1].set_xlabel("Day of year", fontsize=15)
    axs[1].set_ylabel("Total amount of ${\mathrm{NO}_3}^{-}$-$\mathrm{N}$\n" + r"($\mathrm{kg}$ ${\mathrm{NO}_3}^{-}$-$\mathrm{N}$  $\mathrm{ha}^{-1}$)", fontsize=15)
    axs[1].set_ylim(0,200)
    axs[2].plot(df_output["DOY"], df_output.H2O_tot)
    axs[2].set_xlabel("Day of year")
